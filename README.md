...
- LD_MATRIX_FILE (required): Full path and filename prefix of the LD matrix to be computed from the reference panels for each population, in the order corresponding to the GWAS summary statistics files, separated by comma. If the LD file already exists in the specified folder, it will be used directly. Otherwise the LD file will be computed from the reference panel.
