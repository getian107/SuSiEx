...
def write_cs(reg_chr, reg_bp, n_pop, n_cs, snp_dict, beta, ind, n, alpha, cs_bin, cs_purity, pip, out_dir, out_name):
    ...
    with open(out_dir + '/' + out_name + '.cs', 'w') as ff, open(out_dir + '/' + out_name + '.summary', 'w') as ss:
        ...
        if sp.isnan(n_cs):
            ...
            ff.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % \
                ('CS_ID', 'SNP', 'BP', 'REF_ALLELE', 'ALT_ALLELE', 'REF_FRQ', 'BETA', 'SE', 'CS_PIP', 'OVRL_PIP'))
            ss.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % \
                ('CS_ID', 'CS_LENGTH', 'CS_PURITY', 'MAX_PIP_SNP', 'BP', 'REF_ALLELE', 'ALT_ALLELE', 'REF_FRQ', 'BETA', 'SE', 'MAX_PIP'))
            ...
            for ll in range(n_cs):
                ...
                for csid, snp_ll, bp_ll, a1_ll, a2_ll, frq_ll, eff_ll, se_ll, cs_pip_ll, ovrl_pip_ll in \
                    zip([ll+1]*cs_len[ll], snp, bp, a1, a2, frq, eff, se, cs_pip, ovrl_pip):
                    ff.write('%i\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%.3f\t%.3f\n' % \
                    (csid, snp_ll, bp_ll, list2txt(a1_ll), list2txt(a2_ll), arr2f(frq_ll), arr2e(eff_ll), arr2e(se_ll), cs_pip_ll, ovrl_pip_ll))

                idx_max = sp.argmax(cs_pip)
                ss.write('%i\t%i\t%.3f\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%.3f\n' % \
                (ll+1, cs_len[ll], cs_purity[ll], snp[idx_max], bp[idx_max], list2txt(a1[idx_max,:]), list2txt(a2[idx_max,:]), 
                arr2f(frq[idx_max,:]), arr2e(eff[idx_max,:]), arr2e(se[idx_max,:]), cs_pip[idx_max]))
