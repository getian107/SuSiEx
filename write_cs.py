...
def write_cs(reg_chr, reg_bp, n_pop, n_cs, snp_dict, beta, ind, pval, n, alpha, cs_bin, cs_purity, pip, out_dir, out_name):
    ...
    with open(out_dir + '/' + out_name + '.cs', 'w') as ff, open(out_dir + '/' + out_name + '.summary', 'w') as ss:
        ...
        if sp.isnan(n_cs):
            ...
            ff.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % \
                ('CS_ID', 'SNP', 'BP', 'REF_ALLELE', 'ALT_ALLELE', 'REF_FRQ', 'BETA', 'SE', 'PVAL', 'CS_PIP', 'OVRL_PIP'))
            ss.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' % \
                ('CS_ID', 'CS_LENGTH', 'CS_PURITY', 'MAX_PIP_SNP', 'BP', 'REF_ALLELE', 'ALT_ALLELE', 'REF_FRQ', 'BETA', 'SE', 'PVAL', 'MAX_PIP'))
            ...
            for ll in range(n_cs):
                ...
                for csid, snp_ll, bp_ll, a1_ll, a2_ll, frq_ll, eff_ll, se_ll, p_ll, cs_pip_ll, ovrl_pip_ll in \
                    ...
                    (csid, snp_ll, bp_ll, list2txt(a1_ll), list2txt(a2_ll), arr2f(frq_ll), arr2e(eff_ll), arr2e(se_ll), arr2e(p_ll), cs_pip_ll, ovrl_pip_ll))
                ...
                arr2f(frq[idx_max,:]), arr2e(eff[idx_max,:]), arr2e(se[idx_max,:]), arr2e(p[idx_max,:]), cs_pip[idx_max]))
